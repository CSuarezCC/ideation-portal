...
export async function handler(event: AuthEvent): Promise<APIGatewayProxyResultV2> {
  ...
  const result: ScoreDistribution = { campaignId, dimension, buckets: BUCKET_RANGES.map(([rangeLabel, min, max]) => ({ rangeLabel, count: scores.filter(s => { const val = s[key as keyof AggregatedScore] as number; return val >= min && val <= max }).length })) }
