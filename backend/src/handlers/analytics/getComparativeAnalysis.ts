...
import type { ApiGatewayAuthorizerContext, AggregatedScore, ComparativeIdeaRow } from '../../shared/types/index.js'
...
export async function handler(event: AuthEvent): Promise<APIGatewayProxyResultV2> {
  ...
  const ideas: ComparativeIdeaRow[] = await Promise.all(scores.map(async (s) => {
    ...
    return { ideaId: s.ideaId, title: idea?.title ?? 'Unknown', feasibilityAvg: s.feasibilityAvg, impactAvg: s.impactAvg, innovationAvg: s.innovationAvg, compositeScore: s.compositeScore }
  }))
